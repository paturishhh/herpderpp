...
void triggerOnDemand() {
  ...
  //    xbee.print("onDemandValue: ");
  //    xbee.println(onDemand);
  //    xbee.print("Checking port: ");
  //    xbee.println(x);
...
void insertToPacket(byte pQueue[], byte portNumber, unsigned int portData) { // for port data
  ...
  if ((packetPos != (BUFFER_SIZE - 2)) && ((packetPos + 0x03) <= (BUFFER_SIZE - 2))) { //if not full and kapag nilagay mo dapat hindi mapupuno
    ...
    //    xbee.print("Con1: ");
    //    xbee.println((packetPos!= (BUFFER_SIZE-2)));
    //    xbee.print("Con2: ");
    //    xbee.println((packetPos + 0x03) <= (BUFFER_SIZE-2));
...
boolean checkPortConfig() {
  ...
  for (byte x = 0x00; x < PORT_COUNT; x++) {
    ...
    if ((configChangeRegister & bitMask) == bitMask) { // config was changed
      ...
      xbee.print("configType: ");
      xbee.println(configType, HEX); // bakit siya pumapasok sa lahat ng if?
  ...
  if (isService) { // check serial queue
    ...
    if (!isEmpty) { // there are messages
      ...
      if (requestConfig == true) { //if it is waiting for config
        ...
        if ((packetTypeFlag & 0x01) == 0x01) { // request startup config
          ...
          if (serialHead != serialTail) {
            xbee.println("Unexpected packet");
          }
        }
      }
      ...
      if (timerGrant != 0x00) { //check timer grant
        ...
        xbee.println("Check Timer Grant");
        ...
        xbee.print("End loop timerRequest: ");
        xbee.println(timerRequest, HEX);// kahit hindi zero kasi interrupt ito
        ...
        for (byte x = 00; x < PORT_COUNT; x++) {
          ...
          if ((eventRequestMask & eventRequest) == eventRequestMask) {
            ...
            if (conditionReached) {
              ...
              xbee.print("actuator Port: ");
              xbee.println(actuatorPort, HEX);
